...
class sample extends web {//继承后台基类。类名称要与文件名一致
	...
	public function docreategroup(){
        ...
        switch($action){
            case createGroup:
                ...
                DB::query($query);
                $text="设备组添加成功！";
                $sucToPage = $_M[url][site]."data/request_page.php?n=sample&c=sample&a=sample";
        ...
        switch($action){
            case 'addDevice':
                ...
                if($deviceInLib){
                    ...
                    if($onet['device_id']){
                        $text="此序列号已被使用！";
                        $sucToPage = $_M[url][site]."data/request_page.php?n=sample&c=sample&a=doadddevice";
                        require_once $this -> template('own/success');
                    } else{
